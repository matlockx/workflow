 ...
 * `.agent/state/feature-progress.json` inside the target project.
 *
 * IMPORTANT: This file tracks the /feature command's CURSOR POSITION within
 * a workflow, NOT the actual tasks/issues. For task queries ("last task",
 * "open issues"), use the configured backend (bd, task, file) instead.
 *
 * Stage lifecycle:
...
// AIDEV-NOTE: This module tracks /feature command orchestration progress ONLY.
// It is NOT a source of truth for tasks/issues — that's the backend's job.
// When someone asks "what was the last task", query the backend (bd list),
// NOT this file. This file answers "where am I in the /feature workflow?"
...
/**
 * Get the path to feature-progress.json for the current project.
 * @param {string} [projectRoot] - Override for project root (default: cwd)
 * @returns {string}
 */
function featureProgressFilePath(projectRoot) {
  const root = projectRoot || process.cwd()
  return path.join(root, '.agent', 'state', 'feature-progress.json')
}

/**
 * Read and parse feature-progress.json. Returns empty state if missing.
 * @param {string} [projectRoot]
 * @returns {{ activeItems: WorkItem[], history: WorkItem[] }}
 */
function loadFeatureProgress(projectRoot) {
  const filePath = featureProgressFilePath(projectRoot)
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8'))
  } catch (err) {
    if (err.code === 'ENOENT') {
      // AIDEV-NOTE: Also check for legacy workflow.json and migrate if found
      const legacyPath = path.join(projectRoot || process.cwd(), '.agent', 'state', 'workflow.json')
      try {
        const legacyData = JSON.parse(fs.readFileSync(legacyPath, 'utf8'))
        // Migrate: save to new location
        saveFeatureProgress(legacyData, projectRoot)
        // Remove legacy file
        fs.unlinkSync(legacyPath)
        return legacyData
      } catch (legacyErr) {
        // No legacy file either, return empty state
...
/**
 * Write feature progress state to disk atomically (write-then-rename).
 * @param {{ activeItems: WorkItem[], history: WorkItem[] }} state
 * @param {string} [projectRoot]
 */
function saveFeatureProgress(state, projectRoot) {
  const filePath = featureProgressFilePath(projectRoot)
...
function createWorkItem({ issueId, type = 'feature', title }, projectRoot) {
  ...
  const state = loadFeatureProgress(projectRoot)
  ...
  saveFeatureProgress(state, projectRoot)
  return item
}
...
function getAllActiveItems(projectRoot) {
  return loadFeatureProgress(projectRoot).activeItems
}
...
module.exports = {
  ...
  // File helpers (new names)
  featureProgressFilePath,
  loadFeatureProgress,
  saveFeatureProgress,
  
  // Legacy aliases for backward compatibility
  // AIDEV-NOTE: These will be removed in a future version
  workflowFilePath: featureProgressFilePath,
  loadWorkflowState: loadFeatureProgress,
  saveWorkflowState: saveFeatureProgress,
